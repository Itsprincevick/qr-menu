  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-stone-50">
        {/* Hero */}
        <section
  className="pt-32 pb-20 px-6 text-center max-w-2xl mx-auto relative"
  style={{
    backgroundImage: "url('/JAHZBG.webp')",
...
>
  ...
  <div className="relative z-10">
    ...
    <h1 className="text-5xl md:text-6xl font-extrabold text-white leading-tight tracking-tight mb-6">
      Good Food,<br />
      <span className="text-amber-400">Made Fresh.</span>
    </h1>

    <p className="text-stone-300 text-lg leading-relaxed mb-10 max-w-md mx-auto">
    ...
    <Link
      href="/menu"
      className="inline-block bg-amber-500 hover:bg-amber-600 transition-colors text-white font-semibold text-base px-8 py-4 rounded-full"
    >
      Browse Menu →
    </Link>
  </div>
        ...
        {/* Feature cards */}
        <section className="max-w-3xl my-7 mx-auto px-6 pb-24 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={card.title}
              className="bg-[#455041] border border-stone-100 rounded-2xl p-6 text-center shadow-sm"
            >
              <div className="text-3xl mb-3">{card.emoji}</div>
              <h3 className="font-bold text-amber-400 mb-1">{card.title}</h3>
              <p className="text-stone-100 text-sm leading-relaxed">{card.desc}</p>
            </div>
          ))}
        </section>
